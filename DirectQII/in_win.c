...
void IN_MouseMove (usercmd_t *cmd)
{
	...
	if ((in_strafe.state & 1) || (lookstrafe->value && mlooking))
		...
		cl.viewangles[1] -= m_yaw->value * mouse_x;
		...
		case AxisTurn:
			...
			else
			{
				...
				if (fabs (fAxisValue) > joy_yawthreshold->value)
				{
					...
					{
						cl.viewangles[1] += (fAxisValue * joy_yawsensitivity->value) * speed * 180.0;
					}
		...
		case AxisLook:
			if (mlooking)
			{
				if (fabs (fAxisValue) > joy_pitchthreshold->value)
				{
					...
					{
						cl.viewangles[0] += (fAxisValue * joy_pitchsensitivity->value) * speed * 180.0;
					}
				}
			}
			break;
