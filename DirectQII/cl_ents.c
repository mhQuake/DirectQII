...
void CL_ParseProjectiles (void)
{
	...
	for (i=0 ; i<c ; i++)
	{
		...
		VectorCopy(pr.origin, pr.prevorigin);
		...
		if (bits[4] & 128) {
			...
			bits[3] = MSG_ReadByte (&net_message);
			bits[4] = MSG_ReadByte (&net_message);
			pr.prevorigin[0] = ( ( bits[0] + ((bits[1]&15)<<8) ) <<1) - 4096;
			pr.prevorigin[1] = ( ( (bits[1]>>4) + (bits[2]<<4) ) <<1) - 4096;
			pr.prevorigin[2] = ( ( bits[3] + ((bits[4]&15)<<8) ) <<1) - 4096;
		}
		...
		for (j = 0; j < MAX_PROJECTILES; j++) {
			if (cl_projectiles[j].modelindex) {
				if (cl_projectiles[j].num == pr.num) {
					// already present, set up prevorigin for interpolation
					if (!old)
						VectorCopy(cl_projectiles[j].origin, pr.prevorigin);
					cl_projectiles[j] = pr;
					break;
				}
			} else
...
void CL_AddProjectiles (void)
{
	...
	for (i=0, pr=cl_projectiles ; i < MAX_PROJECTILES ; i++, pr++)
	{
		...
		for (j=0 ; j<3 ; j++)
		{
			ent.origin[j] = ent.prevorigin[j] = pr->prevorigin[j] + cl.lerpfrac * (pr->origin[j] - pr->prevorigin[j]);

		}

		if (pr->effects & EF_BLASTER)
			CL_BlasterTrail (pr->prevorigin, ent.origin);
